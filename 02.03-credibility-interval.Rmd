...
` 1. Approximately normally distributed with 
  ...
  $$
  \sigma_{\overline{X}} = \frac{35.33}{\sqrt{3}} = `r 35.33/3^0.5`
  $$
...
2. If the population standard deviation $\sigma$ is known then our belief about the size of the population mean $\mu$ may be represented by the normal distribution of sample means. Suppose we desire a 95\% consistency interval of our conjectures with the data about the size of the population mean. Remember that in our experiment the sample size $n = 3$.Then calculate
...
where $z_{0.025} =$ `NORM.INV(0.025,0,1) = -1.96`, so that
...
where $z_{0.975} =$ `NORM.INV(0.975,0,1) = 1.96`, so that
